...
import (
	...
	apisCluster "github.com/crossplane-contrib/provider-upjet-forgejo/apis/cluster"
	apisNamespaced "github.com/crossplane-contrib/provider-upjet-forgejo/apis/namespaced"
	"github.com/crossplane-contrib/provider-upjet-forgejo/config"
	"github.com/crossplane-contrib/provider-upjet-forgejo/internal/clients"
	controllerCluster "github.com/crossplane-contrib/provider-upjet-forgejo/internal/controller/cluster"
	controllerNamespaced "github.com/crossplane-contrib/provider-upjet-forgejo/internal/controller/namespaced"
	"github.com/crossplane-contrib/provider-upjet-forgejo/internal/features"
	"github.com/crossplane-contrib/provider-upjet-forgejo/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Forgejo").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-forgejo"))
	if *debug {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(scheme), "Cannot add cluster-scoped Forgejo APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(scheme), "Cannot add namespace-scoped Forgejo APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(scheme), "Cannot add CustomResourceDefinition to scheme")

	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		Scheme:           scheme,
		LeaderElection:   *leaderElection,
		LeaderElectionID: "crossplane-leader-election-provider-forgejo",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Forgejo APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Forgejo APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Forgejo controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Forgejo controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Forgejo controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Forgejo controllers")
	}
